...
WunderBook.prototype.showList = function () {
    ...
    if (hadVoted(this)) {
        butLike.attr('src', '/imgs/had-want-icon.png');
        ...
        butLike.attr('src', '/imgs/want-icon.png')
...
function submitWunder(_objBook) {
    ...
    sendData.author = JSON.stringify(_objBook.bookInf.author[0]).replace(/"/ig, "");
    sendData.voter = CookieUtil.get('identify');
...
function submitVote(objBook) {
    ...
    .done(function (resData) {
        ...
        if (res.errcode && res.errcode == 1) {
            ...
            $('.want-ico', objBook.node).attr('src', '/imgs/had-want-icon.png');
